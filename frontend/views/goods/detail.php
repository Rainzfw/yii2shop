<?php
\frontend\assets\CommonAsset::addCssFile($this,Yii::getAlias('@web').'style/goods.css');
\frontend\assets\CommonAsset::addCssFile($this,Yii::getAlias('@web').'style/common.css');
\frontend\assets\CommonAsset::addCssFile($this,Yii::getAlias('@web').'style/jqzoom.css');
\frontend\assets\CommonAsset::addJsFile($this,Yii::getAlias('@web').'js/jqzoom-core.js');
\frontend\assets\CommonAsset::addJsFile($this,Yii::getAlias('@web').'js/goods.js');
?>
<!-- 商品页面主体 start -->
<div class="main w1210 mt10 bc">
    <!-- 面包屑导航 start -->
    <div class="breadcrumb">
        <h2>当前位置：<a href="<?=\yii\helpers\Url::to(['index/index'])?>">首页</a> ><a href="<?=\yii\helpers\Url::to(['goods/index','gcid'=>$goodsinfo->goods_cate_id])?>"><?=$goodsinfo->goodscategory->name;?></a> > <?=$goodsinfo['name']?></h2>
    </div>
    <!-- 面包屑导航 end -->
    ...
    <div class="goods_content fl mt10 ml10">
        <!-- 商品概要信息 start -->
        <div class="summary">
            <h3><strong><?=$goodsinfo->name?></strong></h3>

            <!-- 图片预览区域 start -->
            <div class="preview fl">
                <div class="midpic">
                    <a href="<?=Yii::$app->params['imgUrl'].$goodsinfo->logo;?>" class="jqzoom" rel="gal1">   <!-- 第一幅图片的大图 class 和 rel属性不能更改 -->
                        <img src="<?=Yii::$app->params['imgUrl'].$goodsinfo->logo;?>" alt="" />               <!-- 第一幅图片的中图 -->
                    </a>
                </div>
                ...
                <div class="smallpic">
                    ...
                    <div class="smallpic_wrap">
                        <ul>
                            <li class="cur">
                                <a class="zoomThumbActive" href="javascript:void(0);" rel="{gallery: 'gal1', smallimage: '<?=Yii::$app->params['imgUrl'].$goodsinfo->logo;?>',largeimage: '<?=Yii::$app->params['imgUrl'].$goodsinfo->logo;?>'}"><img src="<?=Yii::$app->params['imgUrl'].$goodsinfo->logo;?>"></a>
                            </li>
                            <?php foreach($gallerys as $gallery):?>
                            <li>
                                <a href="javascript:void(0);" rel="{gallery: 'gal1', smallimage: '<?=Yii::$app->params['imgUrl'].$gallery->img_url?>',largeimage: '<?=Yii::$app->params['imgUrl'].$gallery->img_url?>'}"><img src="<?=Yii::$app->params['imgUrl'].$gallery->img_url?>"></a>
                            </li>
                            <?php endforeach;?>
                        </ul>
                    </div>
            ...
            <div class="goodsinfo fl ml10">
                <ul>
                    <li><span>商品编号： </span><?=$goodsinfo->sn;?></li>
                    ...
                    <li><span>上架时间：</span><?=date('Y-m-d',$goodsinfo->create_time)?></li>
                    <li class="star"><span>商品评分：</span> <strong></strong><a href="">(已有21人评价)</a></li> <!-- 此处的星级切换css即可 默认为5星 star4 表示4星 star3 表示3星 star2表示2星 star1表示1星 -->
                </ul>
                <form id='addcart' action="<?=\yii\helpers\Url::to(['cart/add','gid'=>$goodsinfo['id']])?>" method="post" class="choose">
                    <ul>
                        <li>
                            <dl>
                                <dt>购买数量：</dt>
                                <dd>
                                    ...
                                    <input type="hidden" name="gid" value="<?=$goodsinfo['id'];?>" class="amount"/>
                                </dd>
                            </dl>
                        </li>
        ...
        <div class="detail">
            ...
            <div class="detail_bd">
                ...
                <div class="introduce detail_div none">
                    <div class="attr mt15">
                        <ul>
                            <li><span>商品名称：</span><?=$goodsinfo->name;?></li>
                            <li><span>商品编号：</span><?=$goodsinfo->sn;?></li>
                            <li><span>品牌：</span><?=$goodsinfo->brand->name;?></li>
                            <li><span>上架时间：</span><?=date('Y-m-d H:m:i',$goodsinfo->create_time)?></li>
                        </ul>

                    </div>
                    <div class="desc mt10">
                        <!-- 此处的内容 一般是通过在线编辑器添加保存到数据库，然后直接从数据库中读出 -->
                        <?=$goodsinfo->goodsintro->content?>
                    </div>
                </div>
                <!-- 商品介绍 end -->
